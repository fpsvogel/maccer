...
local keytbl = {["nescio"]="nesciō",["hoc"]="hōc",["hoc1"]="hoc"}
local freqtbl = {["nesciō"]=1,["hoc"]=2,["hōc"]=34}
...
--local command, text = ...
local command, text = arg[1], arg[2]
if command == "macronize" then text = macronize(clearflags(text))
...
--return text
local file = io.open("C:/out.txt", "w")
file:write(text)
file:close()